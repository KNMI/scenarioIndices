#' Calculates a set of daily minimum temperature related indices as they were defined
#' for the KNMI14 scenarios brochure
#' @description      function calculates a set of TN related incides
...
#'
#' @export
TempMinIndices<- function(ifile_tn, index,
                          ofile = NA, scenario,
                          horizon=2030, season,
                          regio.file = NA) {

  if (index!=c("nstFD", "nFD", "nTN", "aTN"))
    stop("index should be one of nstFD nFD nTN aTN")


  input <- TransformTemp(ifile=ifile_tn, var="tn", scenario=scenario,
